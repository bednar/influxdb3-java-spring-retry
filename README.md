...
This project demonstrates how to integrate the [influxdb3‑java](https://github.com/InfluxCommunity/influxdb3-java) client with Spring Boot and Spring Retry.  It exposes a single REST endpoint that accepts a SQL query, executes it against an InfluxDB 3 database and returns the raw rows.  The goal is to provide a reproducible setup for investigating memory leak reports when the Java client is used together with retry semantics.

## Architecture

- **Spring Boot 3.5.7** – the latest stable Spring Boot release as of May 2025【593406577064620†L540-L556】 provides the application framework and auto‑configuration.
- **influxdb3‑java 1.6.0** – the client library used to talk to InfluxDB 3.  Its Maven coordinates are `com.influxdb:influxdb3-java:1.6.0`【342414005464629†L40-L46】.  The `query()` API returns a `Stream<Object[]>` that wraps Apache Arrow buffers.  Streams must be closed or enclosed in a `try-with-resources` block to ensure direct memory is released.
...
This project is intended to reproduce potential memory leak scenarios.  Because the InfluxDB 3 client uses Apache Arrow under the hood, always ensure you close returned streams (for example, by using a `try-with-resources` block as shown in the service implementation) to release off‑heap buffers.  You can run the application with strict JVM limits on direct memory to more easily detect leaks, for example:

```
-Djdk.nio.maxCachedBufferSize=262144 -XX:MaxDirectMemorySize=256M -XX:MaxMetaspaceSize=128M -Xss1M \
-XX:MaxRAMPercentage=75.0 -XX:MaxHeapFreeRatio=40 -XX:MinHeapFreeRatio=20 -XX:ReservedCodeCacheSize=64M
```

Monitoring tools such as `jstat` or `jcmd VM.native_memory` can be used to inspect direct memory usage.
